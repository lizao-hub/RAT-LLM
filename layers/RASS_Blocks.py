...
#         self.encoder = MultiScaleFeatureExtractor(c_in, d_model)
#
#         # --- 缓存 Buffer ---
...
#             feat_wins = df[:, :self.c_in].unfold(0, self.seq_len, self.stride).permute(0, 2, 1)
#             all_feature_windows_list.append(feat_wins)
#
#             # 提取完整部分 (X+Y) 用于检索返回
#             # [N_wins, C_db, SeqLen]
#             orig_wins = df.unfold(0, self.seq_len, self.stride)
...
#
#         # 6. 训练时负采样 (Negative Sampling)
#         # if self.training and self.negative_sampling_ratio > 0:
#         #     replace_mask = torch.rand(B, k_selected, device=device) < self.negative_sampling_ratio
#         #     num_replace = replace_mask.sum().item()
#         #
#         #     if num_replace > 0:
#         #         rand_indices = torch.randint(0, self.total_windows_in_db.item(), (num_replace,), device=device)
#         #         batch_indices = torch.where(replace_mask)[0]
#         #
#         #         rand_scores = score_matrix[batch_indices, rand_indices]
#         #
#         #         topk_scores[replace_mask] = rand_scores
#         #
#         #         # 替换窗口数据
#         #         rand_wins = self.all_original_windows_cache[rand_indices]  # [N_repl, C_db, L]
#         #         rand_wins = rand_wins.permute(0, 2, 1)  # [N_repl, L, C_db]
#         #         selected_windows[replace_mask] = rand_wins
#
#         return topk_scores, selected_windows, topk_scores, selected_windows
